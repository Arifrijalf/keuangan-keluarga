...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    currentUser = user;
    isAdmin =
      typeof LIST_ADMIN !== "undefined" && LIST_ADMIN.includes(user.email);
    isFamilyMember =
      typeof FAMILY_EMAILS !== "undefined" &&
      FAMILY_EMAILS.includes(user.email);
    ...
    if (editTipeSelect)
      editTipeSelect.addEventListener("change", () =>
...
function updateKategori() {
  updatePilihanKategori("tipe", "kategori");
}
...
function bacaDataTransaksi() {
  db.collection("transaksi")
    ...
    .onSnapshot((snapshot) => {
      let html = "";
      let totalSaldo =
        modeTab === "keluarga" ? saldoAwalKeluarga : saldoAwalPribadi;
      let stats = {};
      let daily = {};
      let allDocs = [];
      ...
      allDocs.forEach((data) => {
        ...
        if (modeTab === "keluarga") {
          if (isAdminEntry || isMyEntry) {
            if (!isAdminEntry || data.is_family_trx !== false)
              isValidTab = true;
          }
        } else {
          if (
            isMyEntry &&
            (data.is_family_trx === true && modeTab === "pribadi") === false
          )
            isValidTab = true;
        }
        ...
        if (data.tipe === "pemasukan") totalSaldo += val;
        else totalSaldo -= val;

        let dateObj = data.waktu ? data.waktu.toDate() : new Date();
        if (
          dateObj.getMonth() === filterBulan &&
          dateObj.getFullYear() === filterTahun
        ) {
          if (
            modeTab === "keluarga" &&
            !isAdmin &&
            isAdminEntry &&
            data.kategori === "Setoran Anggota"
          )
            return;

          let tgl = dateObj.getDate();
          if (!daily[tgl]) daily[tgl] = { pemasukan: 0, pengeluaran: 0 };
          if (data.tipe === "pemasukan") daily[tgl].pemasukan += val;
          else daily[tgl].pengeluaran += val;
      ...
      document.getElementById("tampilanSaldo").innerText =
      ...
      docsRev.forEach((data) => {
        ...
        if (
          ...
          const cls =
            data.tipe === "pemasukan"
              ? "border-success border-start border-4"
              : "border-danger border-start border-4";
          const ico =
            data.tipe === "pemasukan"
      ...
      document.getElementById("daftarTransaksi").innerHTML =
        html ||
        '<p class="text-center text-muted mt-3">Belum ada transaksi di bulan ini.</p>';
      renderChart(stats);
...
function renderLineChart(daily) {
  ...
  myLineChart = new Chart(ctx, {
    ...
    options: {
      ...
      maintainAspectRatio: false,
      plugins: {
        legend: { display: type === "semua", labels: { color: textColor } },
      },
      scales: {
        x: {
...
function pantauLangganan() {
  db.collection("langganan").onSnapshot((s) => {
    let h = "";
    const n = new Date(),
      p = `${n.getMonth()}-${n.getFullYear()}`;
    s.docs.forEach((d) => {
      const v = d.data(),
        id = d.id;
      if (modeTab === "keluarga" && v.type !== "keluarga") return;
      if (
        modeTab !== "keluarga" &&
        (v.email_pemilik !== currentUser.email || v.type !== "pribadi")
      )
        return;
      let l = v.riwayat_bayar && v.riwayat_bayar.includes(p),
        bg = l
          ? '<span class="badge bg-success-subtle text-success border border-success rounded-pill">Lunas</span>'
          : n.getDate() > v.tgl_jatuh_tempo
            ? '<span class="badge bg-danger text-white border rounded-pill">Telat!</span>'
            : `<span class="badge bg-warning-subtle text-warning-emphasis border rounded-pill">Tgl ${v.tgl_jatuh_tempo}</span>`,
        btn = l
    ...
    document.getElementById("containerLangganan").innerHTML =
      h ||
      '<div class="col-12 text-center text-muted small py-3">Belum ada tagihan.</div>';
  });
}
window.bukaModalTambahLangganan = () => {
  const s = document.getElementById("tglLangganan");
...
window.simpanLanggananBaru = () => {
  const n = document.getElementById("namaLangganan").value,
    b = parseInt(document.getElementById("biayaLangganan").value),
    t = parseInt(document.getElementById("tglLangganan").value);
  if (!n || !b) return alert("Lengkapi data!");
  db.collection("langganan")
    .add({
      nama: n,
      biaya: b,
      tgl_jatuh_tempo: t,
      type: modeTab === "keluarga" ? "keluarga" : "pribadi",
      email_pemilik: currentUser.email,
      riwayat_bayar: [],
    })
    .then(() =>
      bootstrap.Modal.getInstance(
        document.getElementById("modalTambahLangganan"),
      ).hide(),
    );
};
window.bayarLangganan = (id, n, b) => {
  ...
  Promise.all([
    ...
    db
      ...
      .doc(id)
      .update({ riwayat_bayar: firebase.firestore.FieldValue.arrayUnion(p) }),
  ]).then(() => alert("Lunas!"));
};
window.hapusLangganan = (id) => {
  if (confirm("Hapus?")) db.collection("langganan").doc(id).delete();
};

function pantauTabungan() {
  db.collection("tabungan_goals").onSnapshot((s) => {
    let h = "";
    const b = document.getElementById("btnTambahTabungan");
    if (modeTab === "keluarga") {
      if (isAdmin) b.classList.remove("d-none");
    ...
    document.getElementById("containerTabungan").innerHTML =
      h ||
      '<div class="col-12 text-center text-muted small">Belum ada target.</div>';
  });
}
window.bukaModalTambahTabungan = () => {
  document.getElementById("namaTabungan").value = "";
  document.getElementById("targetTabungan").value = "";
  document.getElementById("pilihanTenor").value = "12";
  document.getElementById("inputTenorManual").classList.add("d-none");
  new bootstrap.Modal(document.getElementById("modalTambahTabungan")).show();
};
window.cekTenorManual = () => {
  const p = document.getElementById("pilihanTenor").value,
    i = document.getElementById("inputTenorManual");
  if (p === "manual") {
...
window.simpanTabunganBaru = () => {
  const n = document.getElementById("namaTabungan").value,
    t = parseInt(document.getElementById("targetTabungan").value);
  let te = document.getElementById("pilihanTenor").value;
  if (te === "manual")
    te = parseInt(document.getElementById("inputTenorManual").value);
  else te = parseInt(te);
  if (!n || !t || !te) return alert("Lengkapi data!");
  db.collection("tabungan_goals")
    .add({
      nama: n,
      target: t,
      tenor: te,
...
window.bukaModalSetorTabungan = (id) => {
  db.collection("tabungan_goals")
    ...
    .then((doc) => {
      ...
      document.getElementById("labelNamaTabungan").innerText = d.nama;
      document.getElementById("infoTarget").innerText =
        `Target: Rp ${parseInt(d.target).toLocaleString()}`;
      document.getElementById("infoSisa").innerText =
        `Sisa: Rp ${(d.target - d.terkumpul).toLocaleString()}`;
      document.getElementById("barProgressTabungan").style.width =
        `${Math.min((d.terkumpul / d.target) * 100, 100)}%`;
      ...
      const l = document.getElementById("listCicilan");
      l.innerHTML = "";
      const c = Math.ceil(d.target / (d.tenor || 1));
      let tm = d.terkumpul;
      for (let i = 1; i <= (d.tenor || 1); i++) {
        let ln = false;
        if (tm >= c) {
          ln = true;
          tm -= c;
        ...
        if (ln)
          dv.innerHTML = `<div class="p-3 bg-success-subtle border border-success rounded d-flex justify-content-between align-items-center mb-2"><div><span class="fw-bold text-success">Bulan ke-${i}</span><div class="small">Rp ${c.toLocaleString()}</div></div><span class="badge bg-success">Lunas</span></div>`;
        else
          dv.innerHTML = `<div class="p-3 bg-light border rounded d-flex justify-content-between align-items-center mb-2"><div><span class="fw-bold text-muted">Bulan ke-${i}</span><div class="small">Rp ${c.toLocaleString()}</div></div><button onclick="prosesBayarCicilan('${id}',${c},'Bulan ke-${i}')" class="btn btn-sm btn-outline-primary rounded-pill">Bayar</button></div>`;
        l.appendChild(dv);
      }
...
window.prosesBayarCicilan = (id, n, k) => {
  ...
  Promise.all([
    db
      .collection("tabungan_goals")
      .doc(id)
      .update({ terkumpul: firebase.firestore.FieldValue.increment(n) }),
    db.collection("transaksi").add(trx),
  ]).then(() => {
    bootstrap.Modal.getInstance(
      document.getElementById("modalSetorTabungan"),
    ).hide();
    setTimeout(() => bukaModalSetorTabungan(id), 500);
  });
};
window.simpanSetoranManual = () => {
  const id = document.getElementById("idTabunganSetor").value,
    v = parseInt(document.getElementById("inputSetoranManual").value);
  if (v > 0) prosesBayarCicilan(id, v, "Setoran Manual");
};
window.hapusTabungan = () => {
  if (confirm("Hapus?"))
    db.collection("tabungan_goals")
      .doc(document.getElementById("idTabunganSetor").value)
      .delete()
      .then(() => location.reload());
};
...
function pantauSaldoKeluarga() {
  db.collection("pengaturan")
    .doc("keuangan_keluarga")
    .onSnapshot((doc) => {
      saldoAwalKeluarga = doc.exists ? doc.data().saldo || 0 : 0;
      if (modeTab === "keluarga") refreshTampilan();
    });
}
function pantauSaldoPribadi() {
  db.collection("pengaturan")
    .doc("saldo_" + currentUser.email)
    .onSnapshot((doc) => {
      saldoAwalPribadi = doc.exists ? doc.data().saldo || 0 : 0;
      if (modeTab === "pribadi") refreshTampilan();
    });
}
...
window.bukaModalSaldo = () => {
  document.getElementById("inputSaldoAwal").value =
    modeTab === "keluarga" ? saldoAwalKeluarga : saldoAwalPribadi;
  new bootstrap.Modal(document.getElementById("modalSaldoAwal")).show();
};
window.simpanSaldoAwal = () => {
  let val = parseInt(document.getElementById("inputSaldoAwal").value);
  let doc =
    modeTab === "keluarga" ? "keuangan_keluarga" : "saldo_" + currentUser.email;
  db.collection("pengaturan")
    .doc(doc)
    .set({ saldo: val })
    .then(() => location.reload());
...
window.bukaModalBudget = () => {
  ["Makan", "Jajan", "Transport", "Belanja", "Tagihan"].forEach(
    (k) => (document.getElementById("budget" + k).value = dataBudget[k] || ""),
  );
...
window.simpanBudget = () => {
  let d = {};
  ["Makan", "Jajan", "Transport", "Belanja", "Tagihan"].forEach(
    (k) => (d[k] = parseInt(document.getElementById("budget" + k).value) || 0),
  );
  db.collection("pengaturan")
    .doc(
      modeTab === "keluarga"
    ...
    .then(() => {
      bootstrap.Modal.getInstance(
        document.getElementById("modalAturBudget"),
      ).hide();
      location.reload();
    });
};
window.resetBudget = () => {
  if (confirm("Reset?"))
    db.collection("pengaturan")
      ...
      .set({ Makan: 0, Jajan: 0, Transport: 0, Belanja: 0, Tagihan: 0 })
      .then(() => location.reload());
};
...
function renderBudgetProgress(exp) {
  ...
  document.getElementById("containerBudget").innerHTML =
    h || '<p class="text-center small text-muted">Belum ada anggaran.</p>';
}
...
window.updateTransaksi = () => {
  db.collection("transaksi")
    .doc(document.getElementById("editId").value)
    .update({
...
window.renderPreviewExport = () => {
  const b = parseInt(document.getElementById("exportBulan").value),
    t = parseInt(document.getElementById("exportTahun").value);
  let totalMasuk = 0,
    totalKeluar = 0,
    count = 0;
  rawDataTransaksi.forEach((data) => {
    ...
    if (dateObj.getMonth() === b && dateObj.getFullYear() === t) {
      if (
        modeTab === "keluarga" &&
        !isAdmin &&
        data.email_pencatat === currentUser.email &&
        data.kategori === "Setoran Anggota"
      )
        return;
      let val = parseInt(data.jumlah);
      if (data.tipe === "pemasukan") totalMasuk += val;
      else totalKeluar += val;
      count++;
    }
  });
  document.getElementById("prevMasuk").innerText =
    `Rp ${totalMasuk.toLocaleString("id-ID")}`;
  document.getElementById("prevKeluar").innerText =
...
window.downloadFileExport = () => {
  ...
  let csvContent =
    "data:text/csv;charset=utf-8,\uFEFFTanggal;Tipe;Dompet;Kategori;Jumlah;Keterangan;Pencatat\n";
  let dataAda = false;
  ...
  rawDataTransaksi.forEach((row) => {
    ...
    if (dateObj.getMonth() === b && dateObj.getFullYear() === t) {
      if (
        modeTab === "keluarga" &&
        !isAdmin &&
        row.email_pencatat === currentUser.email &&
        row.kategori === "Setoran Anggota"
      )
        return;
      let ket = row.keterangan
        ? row.keterangan.replace(/;/g, " ").replace(/\n/g, " ")
        : "-";
      csvContent += `${dateObj.toLocaleDateString("id-ID")};${row.tipe};${row.dompet || "Tunai"};${row.kategori};${row.jumlah};${ket};${row.nama_pencatat}\n`;
      dataAda = true;
    }
  });
  ...
  link.download = `Laporan_${modeTab}_${namaBulan[b]}_${t}.csv`;
